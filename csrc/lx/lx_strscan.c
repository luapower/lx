...
#if !LX_64
#ifndef _WIN32
#define lx_fls(x) ((uint32_t)(__builtin_clz(x)^31))
#else
unsigned char _BitScanReverse(uint32_t *, unsigned long);
#pragma intrinsic(_BitScanReverse)
static uint32_t lx_fls(uint32_t x)
{
...
typedef uint32_t LX_StrScanFmt;
...
static void strscan_double(uint64_t x, LX_Value* o, int32_t ex2, int32_t neg)
{
	double n;

	/* Avoid double rounding for denormals. */
	if (LX_UNLIKELY(ex2 <= -1075 && x != 0)) {
		/* NYI: all of this generates way too much code on 32 bit CPUs. */
#if defined(__GNUC__) && LX_64
		int32_t b = (int32_t)(__builtin_clzll(x)^63);
#else
		int32_t b = (x>>32) ? 32+(int32_t)lx_fls((uint32_t)(x>>32)) :
													(int32_t)lx_fls((uint32_t)x);
#endif
...
static LX_StrScanFmt strscan_hex(const uint8_t *p, LX_Value* o,
											LX_StrScanFmt fmt, uint32_t opt,
...
/* Parse octal number. */
static LX_StrScanFmt strscan_oct(const uint8_t *p, LX_Value *o,
											LX_StrScanFmt fmt, int32_t neg, uint32_t dig)
{
	uint64_t x = 0;
...
static LX_StrScanFmt strscan_dec(const uint8_t *p, LX_Value *o,
											LX_StrScanFmt fmt, uint32_t opt,
...
/* Parse binary number. */
static LX_StrScanFmt strscan_bin(const uint8_t *p, LX_Value *o,
											LX_StrScanFmt fmt, uint32_t opt,
											int32_t ex2, int32_t neg, uint32_t dig)
{
	uint64_t x = 0;
...
static LX_StrScanFmt lx_strscan_scan(const uint8_t *p, LX_Value *o, uint32_t opt)
{
	int32_t neg = 0;

	/* Remove leading space, parse sign and non-numbers. */
	if (LX_UNLIKELY(!lx_char_isdigit(*p))) {
		while (lx_char_isspace(*p)) p++;
		if (*p == '+' || *p == '-') neg = (*p++ == '-');
		if (LX_UNLIKELY(*p >= 'A')) {  /* Parse "inf", "infinity" or "nan". */
			LX_Value tmp;
			setnanV(&tmp);
	...
	/* Parse regular number. */
	{
		...
		if (LX_UNLIKELY(*p <= '0')) {
			if (*p == '0') {
				if (casecmp(p[1], 'x'))
					base = 16, cmask = LX_CHAR_XDIGIT, p += 2;
				else if (casecmp(p[1], 'b'))
					base = 2, cmask = LX_CHAR_DIGIT, p += 2;
			}
			for ( ; ; p++) {
				if (*p == '0') {
					hasdig = 1;
		...
		for (sp = p; ; p++) {
			if (LX_LIKELY(lx_char_isa(*p, cmask))) {
		...
		if (base >= 10 && casecmp(*p, (uint32_t)(base == 16 ? 'p' : 'e'))) {
			...
			fmt = STRSCAN_NUM; p++;
			if (*p == '+' || *p == '-') negx = (*p++ == '-');
			if (!lx_char_isdigit(*p)) return STRSCAN_ERROR;
			xx = (*p++ & 15);
			while (lx_char_isdigit(*p)) {
